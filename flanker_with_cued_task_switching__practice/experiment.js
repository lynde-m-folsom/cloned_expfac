...
function genITIs() { 
	...
	lambda = 1/mean_iti
	iti_array = []
	for (i=0; i < test_length +numTestBlocks ; i++) { //add 3 ITIs per test block to make sure there are enough
...
function getResponseKeys() {
	if (getMotorPerm()==0) {
		...
		return {key: [[71,89],[89,71]], key_name: [["middle finger","index finger"],["index finger","middle finger"]]}
	} else if (getMotorPerm()==2) {
    return {key: [[89,71],[71,89]], key_name: [["index finger","middle finger"],["middle finger","index finger"]]}
  } else if (getMotorPerm()==3) {
    return {key: [[71,89],[71,89]], key_name: [["middle finger","index finger"],["middle finger","index finger"]]}
  }
}
...
var preFileType = '<img class = center src="/static/experiments/flanker_with_cued_task_switching__practice/images/'
...
var response_keys = {key: [[89,71],[89,71]], key_name: [["index finger","middle finger"],["index finger","middle finger"]]}
...
task_switches.unshift({task_switch: 'na', cue_switch: 'na', flanker: jsPsych.randomization.repeat(flanker_types,1).pop()})
...
var pathSource = "/static/experiments/flanker_with_cued_task_switching__practice/images/"
...
var refresh_intro_block = {
    ...
    // }
    var task_switches = jsPsych.randomization.repeat(task_switches_array, practice_length / 4)
    task_switches.unshift({task_switch: 'na', cue_switch: 'na', flanker_type: jsPsych.randomization.repeat(flanker_types,1).pop()}) //changed pop to shift
    var refreshStims = genStims(practice_length + 1)
    stims = refreshStims
	} 
  ...
  loop_function: function(data) {
    ...
    refresh_feedback_text = "<p class = block-text>Please take this time to read your feedback and to take a short break!</p>"
		if (accuracy > accuracy_thresh){	
			...
			refresh_feedback_text += '<p class = block-text>Remember:' + getPromptTaskList()
      ...
      refresh_feedback_text +=
			...
			return true
    // refresh_feedback_text = "<br><p class = block-text>Please take this time to read your feedback and to take a short break!"
...
var flanker_with_cued_task_switching__practice_experiment = [];

flanker_with_cued_task_switching__practice_experiment.push(motor_setup_block);


flanker_with_cued_task_switching__practice_experiment.push(refreshNode);
flanker_with_cued_task_switching__practice_experiment.push(refresh_feedback_block);
flanker_with_cued_task_switching__practice_experiment.push(end_block)