...
var getRefreshFeedback = function() {
	if (getRefreshTrialID()=='instructions') {
		return '<div class = bigbox><div class = picture_box><p class = instruct-text><font color="white">' + '<div class = instructbox>'+ //'<div class = centerbox>'+
...
var getCategorizeFeedback = function(){
	...
	if ((trial_id == 'practice_trial') && (jsPsych.data.getDataByTrialIndex(curr_trial).stop_signal_condition == 'go')){
		if (jsPsych.data.getDataByTrialIndex(curr_trial).key_press == jsPsych.data.getDataByTrialIndex(curr_trial).correct_response){
			...
			return '<div class = upperbox><div class = center-text>Correct!</font></div></div>' + getPromptTextList()
			...
			return '<div class = upperbox><div class = center-text>Incorrect</font></div></div>' + getPromptTextList()
	
		} else if (jsPsych.data.getDataByTrialIndex(curr_trial).key_press == -1){
			
			
			return '<div class = upperbox><div class = center-text>Respond Faster!</font></div></div>' + getPromptTextList()
		...
		if (jsPsych.data.getDataByTrialIndex(curr_trial).rt == -1){
			...
			return '<div class = upperbox><div class = center-text>There was a star.</font></div></div>' + getPromptTextList()
		}
...
var createTrialTypes = function(numTrialsPerBlock){
	...
	for (var x = 0; x < stop_signal_conditions.length; x++){
		for (var j = 0; j < totalShapesUsed; j++){
			stim = {
				stim: shapes[j],
				correct_response: getPossibleResponses()[j][1],
...
var possible_responses = [['index finger', 89], ['middle finger', 71]] //fmri responses - keys: BYGRM = thumb->pinky
...
test_keys(stop_signal_single_task_network__fmri_experiment, [possible_responses[0][1], possible_responses[1][1]])
