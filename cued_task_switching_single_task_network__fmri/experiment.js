...
function getTaskList() {
  return '<ul><li><strong><i>Parity</i> or <i>Odd-Even</i>: Press your ' + getResponseKeys().key_name[1][
      0] + ' key if even and your ' + getResponseKeys().key_name[1][1] + ' key if odd.' +
    '</li><li><i>Magnitude</i> or <i>High-Low</i>: Press your ' + getResponseKeys().key_name[0][
      0] + ' key if the number is greater than 5 and your ' + getResponseKeys().key_name[0][1] +
    ' key if less than 5.</strong></li></ul>'
}

function getPromptTaskList() {
  return '<ul style="text-align:left"><li><i>Parity</i> or <i>Odd-Even</i>: ' + getResponseKeys().key_name[1][0] +
  ' if even and ' + getResponseKeys().key_name[1][1] + ' if odd.' +
  '</li><li><i>Magnitude</i> or <i>High-Low</i>: ' + getResponseKeys().key_name[0][0] +
  ' if >5 and ' + getResponseKeys().key_name[0][1] + ' if <5.</li></ul>'
}
  ...
  switch (curr_task) {
    case 'color':
      if (curr_stim.color == 'orange') {
        return getResponseKeys().key[0][0]
      } else {
        return getResponseKeys().key[0][1]
      }
      break;
    case 'magnitude':
    ...
    case 'parity':
      if (curr_stim.number % 2 === 0) {
        return getResponseKeys().key[1][0]
      } else {
        return getResponseKeys().key[1][1]
      }
  }
}
...
var exp_len = 192 //192 // must be divisible by 4
var numTrialsPerBlock = 48 //48 
  ...
  loop_function: function(data) {
    ...
    if (accuracy < accuracy_thresh){
      refresh_feedback_text +=
          '</p><p class = instruct-text>  Remember: <br>' + getPromptTaskList() 
...
var testNode0 = {
  ...
  var sum_rt = 0
    ...
    if (accuracy < accuracy_thresh){
      feedback_text +=
          '</p><p class = instruct-text>Your accuracy is too low.  Remember: <br>' + getPromptTaskList() 
    }
    if (missed_responses > missed_response_thresh){
      feedback_text +=
