...
var createTrialTypes = function(numTrialsPerBlock, delay){
	first_stims = []
	console.log(delay)
	for (var i = 0; i < delay; i++){ //dropped to 2 because we have a max of 2-back
		//if (i < delay){
	...
	if (numTrialsPerBlock == practice_len) { // for practice blocks, generate the stimuli randomly
		...
		console.log('event type from the design for a given block')
	...
	console.log(stim_len)
	new_stims = []
	for (i = 0; i < stim_len; i++){
		if (i < delay) { //SHIFTED BECAUSE MAX DELAY == 2 NOW
			stim = stims.shift()
			n_back_condition = stim.n_back_condition
			console.log('n back condition here only for first trials when trial lower than 2?')
			console.log(n_back_condition)
			probe = stim.probe
	...
	console.log("printing new_stims length")
	console.log(new_stims.length)
	console.log(new_stims)
	return new_stims
}
...
function getDelay_ref(delays, nback_loc) {
		var delays_copy = delays.slice()

		for(i=0; i < nback_loc.length; i++){
			index = nback_loc[i];

			delays_copy[index] = 3;

		}
		 console.log(delays_copy);
		return delays_copy;
}
/* ************************************ */
...
var delays = $.map(array2_delay, function(v, i) { return [v, array1_delay[i]]; });
//var delays_copy = delays.slice()
var nback_loc =  getAllIndexes(delays, 2)

var delays_ref = getDelay_ref(delays, nback_loc)
...
var design_setup_block = {
	...
	questions: [
		...
		console.log('Original design ITIs for a given permutation, after being transformed from string to number')
		console.log(des_ITIs)
		des_ITIs = insertBufferITIs(des_ITIs, delays)
		console.log('add just 2 buffer ITIs at the beginning of each block!so that there are 2* number of block more trial with random ITIs ')
		console.log(des_ITIs)
		console.log('Print content of what becomes ITiS_stim')
		console.log(des_ITIs.slice(0))
		ITIs_stim = des_ITIs.slice(0)
		ITIs_resp = des_ITIs.slice(0)
		des_events = await getdesignEvents(design_perm)
		console.log("printing delays")
		console.log(delays)
...
console.log(delays)
counter = 1
for (i = 0; i < numTrialsPerBlock + delays_ref[delays_ref.length - counter]; i++) { //was changed from + 3 as delays went from 1:3 to 1:2
	var test_block = {
		type: 'poldrack-single-stim',
		stimulus: getStim,
		is_html: true,
